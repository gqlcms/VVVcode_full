...
Double_t csv (Int_t cud, Double_t x ) // c-jet; cud=1,2,3 for central,up,down; x for pt
{
  ...
  if (cud==1) {  //central
   ...
   else if(x<100e {result=0.892452+0.03569808229804039;}
...
//                std::cout<<IDweight<<"        "<<btagweight_center<<"	"<<trigger_eff<<std::endl;
		...
		tmp_categoryID_channel=-1;// -1 for el; 1 for mu
			...
			if (isAnaHP>0 && (HLT_Ele3>0 || HLT_Ele4>0 || HLT_Ele8>0 || HLT_Mu12>0) && lep==11 && nLooseLep==1){ nID_e = nID_e+1; } else{ isAnaHP=-1; }
			...
			if ((HLT_Ele3>0 || HLT_Ele4>0 || HLT_Ele8>0 || HLT_Mu12>0) && lep==11 && nLooseLep==1 && ptlep1>55 && fabs(etalep1)<2.5 && MET_et>80 && ptVlepJEC > 200.  && jet_pt_puppi>200 && fabs(jetAK8puppi_eta)<2.4 && IDLoose>0 && num_bJet<1 && deltaRlepjet>pi_2 && fabs(delPhijetmet) >2.0 && fabs(delPhijetlep)>2.0 && jetAK8puppi_tau21>0.45 && jetAK8puppi_tau21<=0.75)// && (( jetAK8puppi_sdcorr >0 &&  jetAK8puppi_sdcorr< 150 )) )
			{ isAnaLP=1.; } 
			else{ isAnaLP=-1.; }
			...
			//TTbar control
			if ((HLT_Ele3>0 || HLT_Ele4>0 || HLT_Ele8>0 || HLT_Mu12>0) && lep==11 && nLooseLep==1 && ptlep1>55 && fabs(etalep1)<2.5 && MET_et>80 && ptVlepJEC > 200. && jet_pt_puppi>200 && fabs(jetAK8puppi_eta)<2.4 && IDLoose>0  && num_bJet>0)// &&  jetAK8puppi_sdcorr>0 && jetAK8puppi_sdcorr <150)
			{ isTTBarControl=1.; } 
